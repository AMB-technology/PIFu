...
def main():
    ...
    test_masks = sorted(glob.iglob(args.input_image + "*_grabCutMask.png"))

    for index, (image, mask) in tqdm.tqdm(enumerate(zip(test_images, test_masks))):
        ...
        img_name = extractName(image)
        mask_name = extractName(mask)
